...
def download_data(base_url: str, target_date: datetime, date_range: int, dest_folder: str) -> None:
    """
    Downloads and extracts data for a range of dates around a target date.

    Args:
        base_url (str): The base URL template for the ZIP files.
        target_date (datetime): The target date around which data should be downloaded.
        date_range (int): The number of days before and after the target date to download.
    ...
    # ensure the target date is formatted as YYYY-MM-DD
...
def main() -> None:
    ...
    for currency_pair in config['currency_pairs']:
        for target_date_str in config['target_dates']:
            ...
            download_data(base_url, target_date, date_range, data_folder)
