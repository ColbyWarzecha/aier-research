...
def load_and_prepare_data(master_csv: str, currency_pairs: list) -> pd.DataFrame:
    ...
    for token in currency_pairs:
        ...
        data[f"{token}_log_return"] = np.log(data["Close"]) - np.log(
            data["Close"].shift(1)
        )
...
def visualize_data(
    ...
    data_window = data[
        (data["Open Time"] >= start_date) & (data["Open Time"] <= end_date)
    ]

    plt.figure(figsize=(15, 10))
    sns.lineplot(
        x=data_window["Open Time"], y=data_window["Open"], label="Open", color="blue"
    )
...
def prepare_did_data(
    ...
    for token in currency_pairs:
        df_long[f"treatment_{token.lower()}"] = (
            df_long["asset"] == f"{token}_{return_type}"
        ).astype(int)
    return df_long
...
def visualize_did_results(
    df_did, event_timestamp, event_name, output_folder, use_log_returns=True
):
...
def analyze_event(
    df, event_timestamp, event_name, config, output_folder, use_log_returns=True
):
    currency_pairs = config["currency_pairs"]
    df_did = prepare_did_data(
        df, event_timestamp, currency_pairs, use_log_returns, config["time_window"]
    )
    ...
    visualize_did_results(
        df_did, event_timestamp, event_name, output_folder, use_log_returns
    )
...
def main(config_path: str):
    ...
    for event_date in config["event_dates"]:
        ...
        analyze_event(
            ...
            config,
            output_folder,
            config.get("use_log_returns", True),
        )
