...
def calculate_log_returns(df):
    for token in ['BTCARS', 'BTCUSDT', 'USDCHF']:
        df[f'{token}_log_return'] = np.log(df[f'Close_{token}']) - np.log(df[f'Close_{token}'].shift(1))
    return df
...
def prepare_did_data(df, event_timestamp):
    ...
    df_long = pd.melt(df_window,
                      id_vars=['Open Time'],
                      value_vars=['BTCARS_log_return', 'BTCUSDT_log_return', 'USDCHF_log_return'],
                      var_name='asset',
                      value_name='log_return_value')
    
    df_long['post'] = (df_long['Open Time'] > event_timestamp).astype(int)
    df_long['treatment_btcars'] = (df_long['asset'] == 'BTCARS_log_return').astype(int)
    df_long['treatment_btcusdt'] = (df_long['asset'] == 'BTCUSDT_log_return').astype(int)
    return df_long

# Step 4: Run DiD model
def run_did_model(df, treatment_var):
    formula = f'log_return_value ~ treatment_{treatment_var} + post + treatment_{treatment_var}:post'
...
def visualize_results(df_did, event_timestamp):
    ...
    for asset in ['BTCARS_log_return', 'BTCUSDT_log_return', 'USDCHF_log_return']:
        data = df_did[df_did['asset'] == asset]
        plt.plot(data['Open Time'], data['log_return_value'], label=asset)
    
    plt.axvline(event_timestamp, color='r', linestyle='--', label='Event')
    plt.title('Asset Log Returns Around the Event')
    plt.xlabel('Time')
